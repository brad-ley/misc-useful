...
def calibrate(targ, fit_Hpp_0=True):
    ...
    try:
        ...
        files = sorted([ii for ii in P(targ).iterdir() if ii.name.startswith(
            choice) and ii.name.endswith('exp.txt')], key=lambda x: float(regex.findall(x.name)[0]))
    except IndexError:
    ...
    for i, f in enumerate(files):
        ...
        field = dat[:, 0] * 1e4
        lw.append(np.abs(field[np.argmax(absorptive)] -
        ...
        allax.plot(field, absorptive/np.max(np.abs(absorptive)), label=f'{float(regex.findall(f.name)[0])} mA')


    def forced_fit(x, scale): return fit(x, scale, Hpp)
    fig, ax = plt.subplots()
    ax.plot(mod, lw)
    ...
    if fit_Hpp_0:
        ...
        ax.plot(smoothmod, fit(smoothmod, *popt), label=r"cal$=$" +
                f"{popt[0]:.2f} G/mA\n" + "$H_{pp}=$" + f"{popt[1]:.2f} G")
    else:
        popt, pcov = optimize.curve_fit(forced_fit, mod, lw)
        ax.plot(smoothmod, forced_fit(smoothmod, *popt), label=r"cal$=$" +
                f"{popt[0]:.2f} G/mA\n" + "$H_{pp}=$" + f"{Hpp:.2f} G")
    allax.legend()
    ax.legend()
